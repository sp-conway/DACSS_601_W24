...
We will use four datasets, all of which come with the `ggplot2` package: `economics`, `mpg`, `presidential`, and `txhousing`. We'll start off by converting these into tibbles and storing them in our environment explicitly.
...
With some minor variation from year to year, it looks like the number of unemployed people has generally been increasing. However, this is likely confounded by the total population increasing.
...
Some of these "cities" appear to be counties, and there are also 46 of them. To simplify things, we'll focus on the [5 largest Texas cities](https://www.texas-demographics.com/cities_by_population), which in descending order, are Houston, San Antonio, Dallas, Austin, and Fort Worth.
...
txhousing_1 <- txhousing %>%
  filter(city %in% c("Houston",
                     ...
                     "Austin",
                     "Fort Worth")) %>%
  mutate(date=my(str_c(month,year,sep="-")))
txhousing_1
```
...
Now we can see more significant trends. We see that there was a dip in number of sales around the 2008 housing crisis. This occurred for each city. We also see that although San Antonio is the second largest city in Texas, it had the fourth-largest amount of housing sales among these cities.

Now let's try plotting `total_volume` of sales (the total number of dollars in housing sales), for each year and city.

We also change the `labels` on the y axis to be in `dollar_format`, using the [`scales`](https://scales.r-lib.org/) package.
...
It appears that republicans have spent about 5000 more days in office![^1]

[^1]: Note that this doesn't include Joe Biden's presidency, which as of this posting, has not ended. The Democratic total would be slightly higher, but the trend would be the same.
