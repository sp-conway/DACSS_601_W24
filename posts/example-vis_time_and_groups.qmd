...
This dataset contains a number of economic markers, documented every month beginning in July 1967 and ending in April 2015. We will mostly focus on the variable `unemploy`, which marks the number of unemployed people in thousands.
...
economics1 %>%
  ggplot(aes(date,unemploy_rate))+
  geom_path()+
  scale_x_date(limits=range(economics1$date),breaks="5 years",date_labels = "%Y")+
...
Let's see if we can't examine how `sales`, the number of home sales, has varied over time. We will also compare this separately for each city, to see if some cities have more sales than others. We'll plot `date` on the x axis, `sales` on the y axis, and use the `col` aesthetic to specify that we'll draw a different line for each city. We will draw the line using `geom_path()`.
...
txhousing_total %>%
  ...
  scale_y_continuous(limits = c(0,ceiling(max(txhousing_total$total_sales))))+
...
txhousing_total %>%
  ...
  labs(x="Year",y="Total Volume of Sales")
```
...
presidential_total_dur %>%
  ...
  geom_col()
presidential_total_dur1 <- presidential_total_dur %>%
  mutate(prop=total/sum(total))
presidential_total_dur1 %>%
...
We compute both the lower and upper values of the standard error (+- 1 SE), because we're going to make error bars. This is an advanced topic. We store these statistics in the dataframe `mpg_avgs` .
