...
[^1]: We do already have access to the data because we loaded `tidyverse` , which loads `dplyr` . However, I usually prefer to expliclty load the data into my `R` global environment,
...
ggplot(mtcars1, aes(mpg, hp))+
  ...
  facet_grid(am~vs,scales="free")+
  labs(x="Miles per gallon",y="Horsepower",title="Relationship between mpg & horsepower, split by transmission & engine shape")+
  ggthemes::theme_few()+
