...
We are interested in the distribution of sodium and sugar content in cereals, lets start by checking out a simple histogram - binned into approximately 25 mg ranges. I do this by setting bins equal to max minus the min of the variable.
...
cereal %>%
  ...
  scale_y_continuous("Milligrams (Sodium)", 
    sec.axis = sec_axis(~ . /15, name = "Grams (Sugar)")
  )
...
cereal %>%
  ggplot(aes(reorder(Cereal,-Sodium),Sodium,fill=Type))+
  geom_col()+
  geom_text(aes(label=Sodium),hjust=-.5)+
...
The histogram isn't ideal, we can see the single outlier - but it is hard to get a grasp on the number of cases of pathogens with lower case counts. Perhaps if we rescaled the number of cases to a log or some other scaling function. As we see below, the logging of the x axis is more revealing.
...
Let's see if there's a relationship between `Division` and `Response` - that is, are certain divisions more likely to respond one way compared to other divisons?
...
While learning the true yes/no distribution within the Northern Territory[^1] might change our conclusion about the distribution of responses *within* the Northern Territory, it won't change much about the distribution of responses across Australians overall.

[^1]: We also assume sampling error within a given territory, so forcing participants to respond yes/no won't entirely solve our issue.
...
railroad_summaries <- railroad%>%
  ...
  ungroup() %>%
  filter(str_detect(state,"CANADA",T))
```
...
Woah! There's some real outliers here.
...
This is a very challenging dataset. It is hard to determine what many of the variables indicate, and the [documentation](https://catalog.data.gov/dataset/public-school-characteristics-2017-18-2d7d6) is of little help.

```{r}
pub_sch <- here("posts","_data","Public_School_Characteristics_2017-18.csv") %>%
  read_csv()
pub_sch  
```

However, we can get some information out of the dataset:

-   Each row appears to be a single school `SCH_NAME`.

-   We also see we have categorical variables for district (`LEA_NAME`), City `LCITY`, State (`LSTATE` ), and School level (`SCHOOL_LEVEL`).

-   There are some useful quantitative variables, too: Student-to-Teacher-Ratio (`STUTERATIO`) and Full-time enrollment (`FTE`).

Using the `distinct()` function, we can see that these data are entirely from the 2017-18 academic year.
...
We'll use `select()` to pull out the variables that will make the dataset a bit more manageable. There are likely **many** variables of interest contained in these data that would paint a more nuanced picture of public education in the US, but we may have to email the DOE for the documentation of those.

```{r}
pub_sch1 <- pub_sch %>%
  select(LEA_NAME, SCH_NAME, SCHOOL_LEVEL, 
         LCITY, LSTATE,STUTERATIO, FTE)
```

First, let's make some univariate plots. We'll make histograms of both `FTE` and `STUTERATIO`.

```{r}
ggplot(pub_sch1,aes(FTE))+
  geom_histogram()
ggplot(pub_sch1,aes(STUTERATIO))+
  geom_histogram()
```

Wow, those are skewed histograms. We should probably remove some outliers to make it more readable.

Let's compute data quantiles to see if there are some extreme outliers.

```{r}
quantile(pub_sch1$FTE, probs = seq(0,1,.05), na.rm=T)
quantile(pub_sch1$STUTERATIO, probs = seq(0,1,.05), na.rm=T)
```

It seems like we can safely remove `FTE` values greater than 85 and `STUTERATIO` values greater than 30. Note that this is a "quick and dirty" method of getting histograms with more information in them. For a "real" data analysis (e.g., for a project!), we should be very careful about removing perceived outliers.

```{r}
pub_sch2 <- pub_sch1 %>%
  filter(FTE<85 & STUTERATIO<30)
```
...
pub_sch2 %>%
  ...
  geom_histogram()
pub_sch2 %>%
  ggplot(aes(STUTERATIO))+
  geom_histogram()
```

Wow, those are much more informative! Let's modify these plots to make them a bit cleaner.
...
pub_sch2 %>%
  ...
  labs(x="Full time enrollment")
```

Those are much cleaner plots!

Let's try a quick bivariate visualization. Let's see if there's a relationship between `FTE` and `STUTERATIO`.

```{r}
ggplot(pub_sch2, aes(FTE, STUTERATIO))+
  geom_point()
```

Woah, that's a lot of data. There's maybe too many data points in here.

Let's try grouping by school district and computing the median of both of these values.

```{r}
pub_sch3 <- pub_sch2 %>%
  group_by(LEA_NAME) %>%
  summarise(med_fte=median(FTE),
            med_stuteratio=median(STUTERATIO)) %>%
  ungroup() 
pub_sch3
pub_sch3 %>%
  ggplot(aes(med_fte,med_stuteratio))+
  geom_point()
```

There's clearly not much going on here. FTE & Student teacher ratio may be related, but the relationship is likely more nuanced than can be captured in a simple scatterplot. However, even this knowledge is informative. We can also modify the plot to make it cleaner & "publication ready".
...
pub_sch3 %>%
  ...
  geom_point()+
  labs(x="Median FTE",
       y="Median Student/Teacher Ratio",
       title="2017-18 US FTE & Student-Teacher Ratio")
```
