...
[^1]: I don't expect you to do these calculations on your challenges/homeworks. You can just anticipate the general shape of the data (e.g., the pivoted data will have fewer rows than it does now, and the columns will be...).
...
eggs_long<-eggs_filled%>%
  pivot_longer(cols=contains("large"), # -c(month,year)
               names_to = "egg_type",
               values_to = "avg_price"
  )
eggs_long
...
The final data should potentially be split into two data tables - one with averages for a group of households, the other with proportion information based on income brackets. There is less clarity about how to handle the potential overlap between *race* and *hispanic* group identity information.
